	...
	public abstract static class Entity {
		...
		public <T extends Entity> T collidesWithType(Class<T> type) {
			...
			for (T e : world.getEntitiesOfType(type)) {
				Shape otherShape = e.getTransformedShape();
				if (myShape.getBounds2D().intersects(otherShape.getBounds2D())) {
					Area other = new Area(e.shape);
					Area mine = new Area(this.shape);
